...
const TodoList = () => {
  ...
  return (
    <main>
      ...
      <div className="container flex flex-wrap mx-auto bg-transparent shadow-lg rounded-lg overflow-hidden m-4">
        {sortedTodos.length === 0 ? (
          <p className="w-full text-center text-gray-500 py-10">
            No todos yet. Click + to create your first task.
          </p>
        ) : (
      ...
      {/* Floating Add Button (hidden when modal is open) */}
      {!isModalOpen && (
        <div
          className="fixed bottom-4 right-4 m-8 cursor-pointer transition duration-300 transform hover:scale-110 z-50"
          onClick={() => setIsModalOpen(true)}
        >
          <LuPlusCircle className="text-blue-700 text-5xl md:text-6xl hover:text-white" />
        </div>
      )}
